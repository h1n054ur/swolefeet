...
class SearchMenu(BaseMenu):
    def show(self):
        """Entry point to show the search menu."""
        clear_screen()
        print_header("Search Available Numbers", "🔍")
        self._handle_search_flow()

    def _handle_search_flow(self):
        ...
        # Handle pagination and results display
        self._handle_results_display(results)

    def _handle_results_display(self, results):
        """Handle displaying search results with pagination."""
        current_page = 1
        while True:
            total_pages = self._display_results_page(results, current_page)
    ...
    def _display_results_page(self, results, page_num):
